    ...
    <header className="fixed top-0 left-0 w-full z-50 bg-background border-b border-border">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between h-20 md:h-24">
          ...
          <button 
            onClick={toggleMenu} 
            className="md:hidden w-12 h-12 flex items-center justify-center border border-border bg-transparent text-foreground hover:bg-foreground hover:text-background transition-colors duration-150 ease-fast" 
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-background border-b border-border shadow-2xl">
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`py-4 px-4 font-mono text-sm uppercase tracking-wider border-b border-border ${isActive ? "text-accent bg-muted/50" : "text-foreground"}`}
                >
                  {item.name}
                </Link>
              );
